...
describe("classify", () => {
  ...
  it("matches feature layer files and captures featureName", () => {
    const spec = parseArchitectureSpec(SPEC);
    const m = buildMatchers(spec, spec.roots[0]!);
    const hit = classify("src/lib/orders/application/place-order.ts", m);
    expect(hit?.layerId).toBe("domain");
    expect(hit?.kind).toBe("feature");
    expect(hit?.featureName).toBe("orders");
  });

  it("prefers shared over feature when paths overlap", () => {
    const spec = parseArchitectureSpec(SPEC);
    const m = buildMatchers(spec, spec.roots[0]!);
  ...
  it("returns null for unrelated paths", () => {
    ...
    expect(classify("src/lib/foo.ts", m)).toBeNull(); // file directly under feature_root
  });
});
