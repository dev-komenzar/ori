...
roots:
  ...
  - id: rs
    ...
    public_entry: mod.rs
cross_root:
  - from: { root: rs, path: apps/template-app/src-tauri/src/task_management/slices/complete_task/commands.rs }
    to:   { root: ts, path: apps/template-app/src/task-management/shared/ipc/bindings.ts }
...
# Architecture (ddd-vsa-hex-typescript-tauri template)

This file is the **single source of truth** for both the TypeScript frontend
(`apps/template-app/src/`) and the Rust backend
(`apps/template-app/src-tauri/src/`). Two adapters compile it:
...
| --- | ------------------------------------------- | ----------- | ------- | ----------------- | ------------ | ------------ |
| ts  | `apps/template-app/src`                     | typescript  | eslint  | `task-management` | `slices`     | `index.ts`   |
| rs  | `apps/template-app/src-tauri/src`           | rust        | rust    | `task_management` | `slices`     | `mod.rs`     |

The two roots are bridged by **tauri-specta**, which derives the TS bindings
under `apps/template-app/src/task-management/shared/ipc/bindings.ts` from
the `#[tauri::command]` functions in
`apps/template-app/src-tauri/src/task_management/slices/*/commands.rs`. This
is the only sanctioned cross-root contract; everything else stays inside its
own root. (Rust identifier rules require underscores, hence the
`task_management` / `complete_task` spellings on the Rust side.)
...
```
apps/template-app/src-tauri/src/
...
└── task_management/
    ...
    └── slices/
        ├── mod.rs               # `pub mod complete_task;`
        └── complete_task/       # one folder per backend slice
...
- **Cross-slice direct imports are prohibited.** Use
  `task-management/shared/contracts/` (TS) / `task_management::shared`
...
- **No raw `@tauri-apps/api/core` imports from any UI layer.** Use the
  tauri-specta-generated bindings under `task-management/shared/ipc/`. The
...
- Cross-slice direct imports (e.g., `crate::task_management::slices::other`
  from inside `task_management::slices::complete_task`) are rejected by the
  generated `tests/arch.rs`.
