...
の Tauri 2 デスクトップアプリに ori の feature-sliced DDD scaffold を立ち
上げます。TS と Rust の **言語境界 = cross-feature 隔離境界** として物理的に
...
└── src-tauri/                    # Rust バックエンド
    ...
    └── src/
        ...
        ├── main.rs
        └── features/             # Rust 側 feature-sliced
            ├── mod.rs
            ├── shared/           # cross-feature primitives (AppError, DomainEvent)
            └── tasks/
...
- **cross-feature 直接 import 禁止**。コラボは `shared/` の contracts か domain events 経由
- **各 feature の公開面は 1 つだけ**: TS は `index.ts`、Rust は `mod.rs`
...
ori feature new <feature-id>          # ドメインから feature を切り出す
/ori-flow <feature-id>                # 7-phase TDD
```

feature が TS 側のみで完結する場合は `src/lib/<feature>/` のみを増やす。
Rust 側にも反映が必要な場合は `src-tauri/src/features/<feature>/` を同じ
