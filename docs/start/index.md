...
(`.apm/contexts/patterns/<pattern>/`) と **アーキテクチャ adapter** を組み合わせ、
...
3. `/ori-arch` — pattern / stack を対話で決定し、対応する
   `.apm/contexts/patterns/<pattern>/stacks/<stack>/architecture.md.tpl` を render して
   target の `.ori/architecture.md` 1 ファイルだけを書き出す

`example-slice/` (`.apm/contexts/patterns/<p>/stacks/<s>/example-slice/`) は target に
...
| ------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------- | ----------------------------------------- |
| **TypeScript (web/Node)**                   | ✅ available  | [`ddd-vsa-hex/stacks/typescript/`](../../.apm/contexts/patterns/ddd-vsa-hex/stacks/typescript/)              | [`@ori-ori/arch-adapter-eslint`](../../packages/arch-adapter-eslint)      | [typescript-web.md](./typescript-web.md)  |
| **TypeScript + Rust (Tauri 2)**             | ✅ available  | [`ddd-vsa-hex/stacks/typescript-tauri/`](../../.apm/contexts/patterns/ddd-vsa-hex/stacks/typescript-tauri/)  | eslint (TS) + [`arch-adapter-rust`](../../packages/arch-adapter-rust)     | [tauri-v2.md](./tauri-v2.md)              |
...
1. `.apm/contexts/patterns/<pattern>/stacks/<stack>/architecture.md.tpl` を追加 (placeholder は `{{APP_NAME}}` / `{{BC_NAME}}` 等)
2. `.apm/contexts/patterns/<pattern>/stacks/<stack>/example-slice/` に worked sample を追加 (AI が `/ori-flow new-slice` で参照する study material)
