...
| ------------------------------------------- | ------------- | ------------------------- | ------------------------------------------------------------------------- | ----------------------------------------- |
| **TypeScript (web/Node)**                   | ✅ available  | `ddd-vsa-hex-typescript`       | [`@ori-ori/arch-adapter-eslint`](../../packages/arch-adapter-eslint)      | [typescript-web.md](./typescript-web.md)  |
| **TypeScript + Rust (Tauri 2)**             | ✅ available  | `ddd-vsa-hex-typescript-tauri` | eslint (TS) + [`arch-adapter-rust`](../../packages/arch-adapter-rust)     | [tauri-v2.md](./tauri-v2.md)              |
...
スタックごとに違うのは `/ori-arch` で選ぶ pattern / framework のペア（=
展開される template）だけで、その後のワークフローは共通です。スタック固有の
