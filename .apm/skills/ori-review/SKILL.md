...
- 入力：
  ...
  - `.ori/config.yaml`（`workspace.apps:`）
  - 実装 + テスト：`apps/<app>/src/<bc>/slices/<slice-id>/{domain,application,infrastructure,presentation,tests}/`
  - BC 共有：`apps/<app>/src/<bc>/{domain,shared/contracts/events}/`（slice が touch した範囲のみ）
...
2. **`ori-reviewer` agent を fresh context で spawn**：
   ...
   - 指定するファイルセット：`.ori/slices/<id>/{spec.md,manifest.yaml}`、`apps/<app>/src/<bc>/slices/<slice-id>/{tests,domain,application,infrastructure,presentation}/`、manifest.derives_from の domain docs
