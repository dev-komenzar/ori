...
ユーザが `/ori-review <feature-id>` を呼ぶ、または `/ori-flow` 内部から phase 6 として起動した際に、**`ori-reviewer` agent を fresh context で spawn し、その出力を捌く薄いラッパー**として動作します。**スキル本体はメイン session で動き、Bash 経由で reviewer agent を起動**します。
...
- 入力：
  ...
  - `.ori/features/<id>/tests/`
  - 実装：`src/contexts/<bc>/`（feature が触れた領域）
  - 関連ドメイン文書：`manifest.derives_from`
- 出力：
  - `.ori/features/<id>/review.md` — reviewer agent の指摘ログ
...
1. **前提確認**：
   ...
   - `pnpm -F <feature-pkg> test` が GREEN であることを確認
2. **reviewer 起動コマンド**を Bash で実行：
   ```bash
   ori feature run <feature-id> --phase review
   ```
   - CLI 内部で `.apm/agents/ori-reviewer.md` を読み、`capability: reasoning` で割り当てられた model を fresh context で起動
   - reviewer 側に渡される入力：spec.md / tests / 関連 src / 関連 domain docs / git diff（直近 phase 分）
3. **reviewer の出力を受け取る**：
   - `.ori/features/<id>/review.md` に書き込まれる
   - 形式：
     ```markdown
     ## Findings
     ...
     ```
4. **指摘の処理**：
   - **指摘ゼロ** → phase 6 完了。`bd close ori-review-<feature-id>` で完了
...
5. **差し戻し後の再 review**：
   ...
   - **2 周目で再度指摘が出た場合は停止**し human flag：
     ```bash
     bd human ori-review-<feature-id> --reason="review loop reached 2nd pass; needs human arbitration"
     ```
6. **完了**：
   - `.ori/features/<id>/review.md` を commit
   - `bd close ori-review-<feature-id> --reason="review passed; <N> findings addressed in <N> patches"`
...
- **LOW** src/contexts/note-capture/application/capture-auto-save.ts:
...
- **スキル本体はメイン session**：reviewer は CLI が agent として spawn する
...
- **メインパス**：`/ori-finalize <feature-id>` — phase 7。dirty 解除と必要に応じた proposal sync
