...
3. **各画面について対話**：
   ...
   - **{#depended-by}** — この画面を呼び出す上位画面 / 機能（後の Phase 11b で UI feature 分け）
4. **型 → UI コントロールの写像（既定）**：
   | 型 | UI コントロール候補 |
   |----|------------------|
...
6. **ファイル分割で書き出す**
7. **`ori lint .ori/domain/ui-fields/` を実行**して schema 検証
...
- **通常パス**：`/ori-ddd-11b-ui-grouping` — 各画面の `depended_by` を確定し UI feature 群を切り出す
- **早期切上げパス**：1 画面のみで完結する小規模プロジェクトでは 11b をスキップし `ori feature new ui-<id>` を直接作る
