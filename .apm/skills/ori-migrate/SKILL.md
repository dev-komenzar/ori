...
8. **slice / page の一括 scaffold 提案**：
   - Phase 9 検出時：`workflows/index.md` の workflow を全列挙
   - Phase 11b 検出時：`page-groups.md` (or 旧 `feature-groups.md`) の page を全列挙
   - ユーザに以下を提示：
     ```
     以下を scaffold しますか？
   ...
   - 承認時に Bash で実行：
     ```bash
     ori slice new capture-auto-save
     ori slice new edit-past-note-start
...
- **既存テスト / src の取り込み**：別 issue として `ori slice ingest <slice-id>` を提案（後続 phase）
