---
name: ori-derive
description: /ori-flow phase 1。manifest の derives_from とドメイン文書から feature spec.md を合成する
---

ユーザが `/ori-derive <feature-id>` を呼ぶ、または `/ori-flow` 内部から phase 1 として起動した際に、**該当 feature の `spec.md` をドメイン文書から派生**します。**派生のみ。実装は phase 4 の責務**。

## 引数

- `feature-id`：対象 feature の id（`.ori/features/<id>/` が存在する事を前提）

## 役割

- **派生器**：`manifest.yaml` の `derives_from:` に列挙されたドメイン section を読み、feature 単位の spec に再構成
...
- 入力：
  - `.ori/features/<id>/manifest.yaml`（必須。`derives_from:` を持つ）
  - `manifest.derives_from` に列挙されたドメイン section（例：`domain/aggregates.md#note-aggregate`）
- 出力：`.ori/features/<id>/spec.md`
  - frontmatter: `coherence.source: derived`、`upstream:` に派生元 section を列挙、`hash:` に派生元のスナップショットハッシュ
  - 必須 H2 セクション（**`.apm/instructions/feature-spec.instructions` 準拠**）

## 必須セクション（feature-spec.instructions 準拠）

| H2 | id | 内容 |
|----|----|------|
| `## 概要 {#overview}` | overview | この feature が解く問題、対応する workflow / UI |
| `## 入出力 {#io}` | io | 入力（command / form）と出力（event / view）の型 |
| `## 不変条件 {#invariants}` | invariants | feature 完了時に成り立つ条件（domain 不変条件 + feature 固有制約） |
...
1. **feature 存在確認**：
   ```bash
   ori feature exists <feature-id>
   ```
   - 存在しない場合、**自動で scaffold しない**。代わりに type-recovery を試す：
     ```bash
     ori feature suggest <feature-id>
     ```
     近い既存 id（fuzzy match）があればユーザに「これですか？」と問い、Yes なら正しい id で再開、No なら `ori feature new` の実行をユーザに**確認**してから進める
2. **manifest.yaml の読み込み**：`.ori/features/<id>/manifest.yaml` を Read。`derives_from:` が空ならエラー停止し「先に `ori-ddd-*` で domain を整備するか、manifest に upstream を追記してください」と案内
3. **upstream section の取得**：`ori coherence resolve <feature-id>` を Bash で実行し、派生元 section のテキスト＋ハッシュを取得
...
6. **`ori lint .ori/features/<id>/spec.md` を実行**：
   - 必須 H2 5 種が揃っているか
   - 全 H2/H3 に `{#id}` があるか
...
8. **beads issue 更新**：
   ```bash
   bd update ori-derive-<feature-id> --status=closed --notes="spec.md generated from <N> upstream sections"
   ```
...
ユーザの入力中に Note を自動保存する feature。空白のみの本文は破棄。
...
- **自動 scaffold は禁止**：feature が存在しなくても勝手に `ori feature new` を呼ばない（ユーザ確認必須）
...
- **メインパス**：`/ori-plan <feature-id>` — phase 2。下流 phase（test-red / impl-green / refactor / review）の beads issue description を埋める
