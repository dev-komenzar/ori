...
// packages/skills/ori-flow/src/scenario-status.ts
...
function nextScenarioPhase(current) {
...
function isValidScenarioPhase(value) {
  return SCENARIO_PHASES.includes(value);
}
function formatEpicId(kind, id) {
  return `ori-${kind}-${id}`;
}
...
function mapStateToPhaseStatus(s) {
  switch (s) {
    case "started":
      return "in_progress";
    case "done":
...
function normalizePhaseRecord(raw) {
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    ...
    const validState = ["pending", "in_progress", "done", "failed"].includes(state) ? state : "pending";
    return {
      state: validState,
...
function normalize(raw, id) {
  ...
  const rawBeads = raw.beads && typeof raw.beads === "object" && !Array.isArray(raw.beads) ? raw.beads : {};
  const epic = typeof rawBeads.epic === "string" ? rawBeads.epic : formatEpicId("scenario", id);
  const currentPhase = typeof rawBeads.current_phase === "string" && isValidScenarioPhase(rawBeads.current_phase) ? rawBeads.current_phase : null;
  const rawCompletion = Array.isArray(rawBeads.completion) ? rawBeads.completion : [];
  const completion = [];
  ...
  const rawPhases = raw.phases && typeof raw.phases === "object" && !Array.isArray(raw.phases) ? raw.phases : {};
  const phases = {};
  for (const p of SCENARIO_PHASES) {
    if (p in rawPhases) {
...
function applySet(status, phase, state, now) {
...
async function main() {
  ...
  if (cmd === "show") {
    ...
    if (jsonFlag) {
      ...
      console.log((0, import_yaml.stringify)(status));
    }
    return;
  }
  ...
  await writeFile(statusPath, (0, import_yaml.stringify)(updated), "utf8");
  const phaseStatus = updated.phases[phaseRaw];
  const summary = phaseStatus ? `${phaseRaw} \u2192 ${phaseStatus.state}${phaseStatus.completed_at ? ` (completed: ${phaseStatus.completed_at})` : ""}` : `${phaseRaw} \u2192 ${mappedState}`;
