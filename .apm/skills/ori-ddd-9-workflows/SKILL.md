...
[1] 一括で作成（ori slice new <each-id> を順次実行）
[2] 個別に選択
[3] スキップ（後から手動で `ori slice new` を呼ぶ）
```
...
各 slice について beads issue（`ori-slice-<id>` epic + 7 phase tasks）が作成されます。
...
- **scaffold パス**：slice を `ori slice new` で一括作成（上記参照）
