---
name: ori-bug
description: 人間が見つけたバグを README の 4 ケース（domain / impl / spec / cross-feature）に triage し、対応する動線を案内する。実行はしない、ルーティングのみ
---
...
- **triage 担当**：4 つの質問でバグを 4 ケース（domain / impl / spec / cross-feature）に分類
...
| **4** | **複数 feature の統合バグ** | 単体は OK だが組み合わせで破綻 | 新規 feature 作成 |
...
3. **「spec の規定通り impl が動かない？」**
   - Yes → **ケース 2（impl bug）**
   - No → 次へ
4. **「単一 feature の範囲を超える？」**
   - Yes → **ケース 4（cross-feature bug）**
...
推奨動線：
  ...
  2. ori sync                         ← dirty 化された feature を一覧表示
  3. /ori-flow <dirty-feature-1>      ← 影響 feature を順次再走

ヒント：複数 feature が dirty 化される可能性が高い。最も影響の大きい
feature から /ori-flow で再 derive することを推奨。
```
...
推奨動線：
  1. .ori/features/<id>/tests/ に失敗テストを追加（whitespace / unicode 等）
  2. pnpm test                      ← RED 確認
  3. ori feature run <id> --phase impl-green --reason "manual bug: <概要>"
  4. ori feature run <id> --phase review (推奨)
...
### ケース 4（cross-feature bug）

```
分類：統合バグ。複数 feature にまたがる。

推奨動線：
  1. /ori-distill phase=workflows   ← Phase 9 に戻り欠落シナリオを追加
  2. ori feature new <integration-feature-id> --type workflow
  3. /ori-flow <integration-feature-id>

ヒント：既存 feature を編集せず、新規 feature として境界を明確にする。
...
- **ケース 4 と分類された場合**：`/ori-ddd-9-workflows` 再走 → `ori feature new` → `/ori-flow`
