...
2. **`.ori/` skeleton を作成**：
   ...
   このスクリプトは過渡期実装として `ori init` CLI を呼ぶ thin wrapper（ori-05r）。
   PATH に `ori` が無ければ `pnpm dlx @ori-ori/cli` / `npx -p @ori-ori/cli` に fallback。
   将来は init core を npm パッケージに切り出し、CLI 経由を廃止する予定
   (ori-execution-model-shift-2026-06-03)。
