...
description: distill-ddd Phase 9（Workflows）。DMMF pipeline をファイル分割（workflows/index.md + <id>.md）で記述し、workflow feature の scaffold を提案する
---
...
6. **`ori lint .ori/domain/workflows/` を実行**して schema 検証
7. lint 失敗時は **1 回だけ** 自動修正を試み、それでも失敗ならユーザに判断を委ねる

### Phase 完了時：workflow feature の一括 scaffold 提案

Phase 9 完了後、index.md の workflow 一覧を読み上げ、ユーザに以下を確認：

```
以下の workflow を feature として scaffold しますか？

  - capture-auto-save
  - edit-past-note-start
  - switch-edit-target

[1] 一括で作成（ori feature new <each-id> --type workflow を順次実行）
[2] 個別に選択
...
各 feature について beads issue（`ori-feature-<id>` epic + 7 phase tasks）が作成されます。
...
- **scaffold パス**：workflow feature を `ori feature new` で一括作成（上記参照）
  - 作成後すぐ `/ori-flow <feature-id>` で 7-phase 実装を開始できる
