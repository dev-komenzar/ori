---
name: ori-test-red
description: /ori-flow phase 3。spec.md のテスト観点から failing test を apps/<app>/src/<bc>/slices/<id>/tests/ に書き起こす（RED 確認まで）
---

ユーザが `/ori-test-red <slice-id>` を呼ぶ、または `/ori-flow` 内部から phase 3 として起動した際に、**該当 slice の `apps/<app>/src/<bc>/slices/<slice-id>/tests/` 配下に failing test を書く**。**impl は書かない**。RED が観測できた時点で完了。
...
- 出力：
  - `apps/<app>/src/<bc>/slices/<slice-id>/tests/<topic>.test.ts`（1 観点 1 ファイル基本、関連は集約可）
  - test runner: vitest
...
- **`.ori/slices/<id>/src/` への出力は禁止**。`.ori/slices/<id>/` は SSoT メタ専用（manifest.yaml / spec.md / status.yaml / notes.md 等のみ）。code および tests は必ず `apps/<app>/src/<bc>/slices/<slice-id>/` 配下に置く
- impl 配置先は skill 起動時に Bash で `mkdir -p apps/<app>/src/<bc>/slices/<slice-id>/tests` を実行し、出力先を強制的に解決済み path に固定する
...
3. **テストファイルの構成**：
   ```
   apps/<app>/src/<bc>/slices/<slice-id>/tests/
...
5. **`pnpm test --filter <slice-id>` 相当を Bash で実行**して RED を確認：
   ```bash
   pnpm -F <app> test apps/<app>/src/<bc>/slices/<slice-id>/tests
   ```
6. **GREEN 観測時：強制停止**
...
- **`.ori/slices/<id>/` には絶対書かない**：code と tests は必ず `apps/<app>/src/<bc>/slices/<slice-id>/` 配下
