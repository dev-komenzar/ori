...
ユーザが `/ori-test-red <feature-id>` を呼ぶ、または `/ori-flow` 内部から phase 3 として起動した際に、**該当 feature の `tests/` 配下に failing test を書く**。**impl は書かない**。RED が観測できた時点で完了。

## 引数

- `feature-id`：対象 feature の id（`.ori/features/<id>/spec.md` が存在する事を前提）
...
- 入力：
  - `.ori/features/<id>/spec.md`（phase 1 で生成済み）
  - `.apm/instructions/ddd-test.instructions`（テスト規約）
- 出力：
  - `.ori/features/<id>/tests/<topic>.test.ts`（1 観点 1 ファイル基本、関連は集約可）
  - test runner: vitest
...
|--------|------|
| runner | vitest |
| 命名 | `describe('feature:<feature-id>', ...)` を必ず最外殻に |
...
1. **前提確認**：
   - `.ori/features/<id>/spec.md` の `## テスト観点 {#test-perspectives}` を Read
...
5. **`pnpm test --filter <feature-id>` 相当を Bash で実行**して RED を確認：
   ```bash
...
6. **GREEN 観測時：強制停止**
   ...
   - bd issue にコメントを残して human flag：
     ```bash
     bd update ori-test-red-<feature-id> --notes="test was GREEN at first run — spec gap suspected"
     bd human ori-test-red-<feature-id> --reason="GREEN-on-first-run anomaly"
     ```
   - ユーザに「spec の観点が impl 済みの動作と合致しているか確認してください」と促す
7. **RED 観測時：phase 3 完了**
   - 失敗テストの一覧を beads notes に記録：
     ```bash
     bd update ori-test-red-<feature-id> --notes="N failing tests written: ..."
     ```
   - `bd close ori-test-red-<feature-id>` で完了
...
```ts
// .ori/features/capture-auto-save/tests/capture-auto-save.test.ts
import { describe, it, expect } from 'vitest';
import { captureAutoSave } from '../src/capture-auto-save';

describe('feature:capture-auto-save', () => {
...
```ts
// .ori/features/capture-auto-save/tests/capture-auto-save-vo.property.test.ts
import { describe, it } from 'vitest';
import fc from 'fast-check';
import { NoteBody } from '../src/vo/note-body';

describe('feature:capture-auto-save VO', () => {
...
- **メインパス**：`/ori-impl-green <feature-id>` — phase 4。失敗テストを GREEN にする最小実装
