...
最後に `.apm/contexts/patterns/<pattern>/stacks/<stack>/architecture.md.tpl` を render して
...
`example-slice/` (`.apm/contexts/patterns/<pattern>/stacks/<stack>/example-slice/`) は
...
3. **stack を選んでもらう**：
   ...
   - **pattern × stack のマトリクス**：`.apm/contexts/patterns/<pattern>/stacks/<stack>/architecture.md.tpl` が存在する組合せのみ有効。`render-architecture.js` は未知組合せで exit 2 + 利用可能な値を列挙する
...
6. **`.ori/architecture.md` を render する**：
   ...
   - `--patterns-dir <dir>` patterns root (default は skill bundle 隣接の `.apm/contexts/patterns/`)
...
7. **example-slice/ への参照導線を AI に思い出させる**：
   - 初回 slice を作るとき `/ori-flow new-slice <id>` で必ず
     `.apm/contexts/patterns/<pattern>/stacks/<stack>/example-slice/` を **読んでから**
...
- **patterns/ 探索順** (`render-architecture.js`)：
  1. `--patterns-dir <dir>` 引数
  2. `$ORI_PATTERNS_DIR` 環境変数
  3. skill bundle 隣接 (`.apm/contexts/patterns/` — apm install 後の relative path)
  4. ori repo dev fallback
