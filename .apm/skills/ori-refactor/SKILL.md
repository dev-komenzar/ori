...
- 入力：
  - `apps/<app>/src/<bc>/slices/<slice-id>/{domain,application,infrastructure,presentation}/`（phase 4 の出力）
  - `apps/<app>/src/<bc>/slices/<slice-id>/tests/`（GREEN 状態、変更禁止）
  - `.ori/slices/<id>/manifest.yaml`（`bc:` と `app:` の解決）
  - `.ori/config.yaml`（`workspace.apps:`）
  - `.apm/instructions/ddd-typescript.instructions`
- 出力：
  - 同じ `apps/<app>/src/<bc>/slices/<slice-id>/...` を tidy（差分のみ）
