...
function buildElements(matchers) {
...
function buildElementTypeRules(spec, root, rules) {
  const set = spec.layer_sets[root.layer_set];
...
function layerFileGlob(spec, root, matchers, layerId) {
  const set = spec.layer_sets[root.layer_set];
  if (!set) return null;
...
function buildForbiddenBlocks(spec, root, matchers) {
  const set = spec.layer_sets[root.layer_set];
  if (!set) return [];
  const blocks = [];
